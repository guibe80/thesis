...
\begin{table}
	...
	\begin{tabular}{ccp{4.5cm}}
		\hline 
		\textbf{Instrument} & \textbf{Method} & \textbf{Measurement} \\ 
		\hline 
		ECG & Sense of electrical charges in heart & Electrocardiogram \\ 
		\hline 
		PPG & Optical & Measurement of changes of volume in vascular bed \\ 
		\hline 
		LDF & Optical & Measurement flow in capillary bed (Cell level) \\ 
		\hline 
		iPG & Electrical & Measurement of changes of volume in a segment \\ 
		Doppler Ultrasound & Electromagnetic & Measurement of flow speed \\ 
		\hline 
	\end{tabular}  
\end{table}

\todo{check how to align this table properly}
...
\begin{align}
	\label{eq:v_e}
	V_e =\frac{l \times (C_1^2+C_1 \times C_2 + C_2^2)}{(12 \times \pi)}
\end{align}
...
\todo{I am not 100\% sure that has two outputs}
...
\begin{align}
	\label{eq:fshift}
	f_D = \frac{V_{UD} \times 8 KHz}{3.5V}
\end{align}  
...
\begin{align}
	\label{eq:doppler}
	v = \frac{f_D \times C}{2 f_O \times Cos(\theta)}
\end{align}
...
\todo{find the right name of a holster and grabber}
...
After the conversion of impedance from volts, it is possible to compute changes volume and flow. Chapter \ref{chapter3} describes in depth the governing equations. Also, the analysis of the impedance plethysmography waveform will provide further data of the blood's haemodynamics. Such as the relation between systole and diastole cycles with the iPG signal.
...
\todo{Improve this paragraph, explaining both methods}
...
\todo{double check a reference to Medical Background how this works. Create a reference to this section}
...
\todo{reference papers where venous occlusion plethysmography is used}
\todo{Create a graph explaining how the gradient of the signal changes during venous occlusion}
...
\todo{Confirm the nomenclature of the ports}
...
Making these values more meaningful, they need to be converted in a time interval per minutes. Also, the limb blood flow should be expressed in units of \si{\milli\litre} per \SI{100}{\milli\litre} of limb section. In this case, the equation can be declared as \ref{eq:QL}.
...
\todo{Verify the name of the port and also the section where should be referenced}
...
\todo{Add picture of waveform with reference points as expressed in the paragraphs RM1, RM2, RM3, RM4 and RM5}
...
\todo{Find a reference between echocardiography, dicrotic notch and isovolumetric relaxation of the heart. Find paper relating heart cycle with the process during diastolic process} 
...
\begin{align}
	\label{eq:exht}
	EXHT = (R_{M3}-R_{M1}) + \frac{(t_{M3}-t_{M1})(R_{M2}-R_{M3})}{t_{M3}-t_{M2}}
\end{align}
...
Next replacing $EXHT$ and $HR$ from equations \ref{eq:RB} and \ref{eq:exht} respectively, into Nyober's equation (see \ref{eq:Volume}) is possible to calculate the blood flow. However, computing it as blood flow with units (\si{\milli\litre\per\minute}), the HR must be included in the equation. In the end, Nyober's equation can be re-written as \ref{eq:bf}. 
...
\begin{align}
	\label{eq:bfve}
	BFVE = \frac{BF}{V_e} \times 1000ml \tagaddtext{[\si{\ml / \min.\liter}]}
\end{align}

where BFVE is the blood flow per litre of volume \si{\milli\litre\per\min.\liter} , BF is the blood flow found in \ref{eq:bf}, $V_e$ is the forearm's segment volume in \si{\cubic\centi\meter} from equation \ref{eq:v_e} and \SI{1000}{ml} is the scaling factor to convert the volume into litre.
...
\begin{align}
	\label{eq:bfpct}
	BF_{100ml} = \frac{BF}{V_e} \times 100ml \tagaddtext{[\si{ml/min.100.ml}]}
\end{align}
