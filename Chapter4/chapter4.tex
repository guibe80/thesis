...
Once data were stored in a local LVM file, it was post-processed in Matlab~\cite{MATLAB:2016}. In detail, a GUI Matlab program was created capable of comparing signals, apply filters, find peaks and windowed sections of data. A front-end of the application can be seen in Fig. xxx.
...
Additional filters were applied removing unwanted high frequency components from attained signals. $V_{ZDC}$ and $V_{IDC}$ signals were filtered using a \nth{10} order low-pass IIR $(f_c=5 Hz)$ and a \nth{4} order band-stop IIR filter $(f_L=$\SI{0.1}{\hertz} and $f_H=$\SI{0.5}{\hertz} $)$ was used to remove respiration and motion artifact. $V_{ZAC}$ signal was processed with a \nth{10} order band-pass IIR filter $(f_L=$ \SI{0.5}{\hertz} and $f_H=$ \SI{5}{\hertz}$)$ removing motion, respiration artefact and high frequency noise.
