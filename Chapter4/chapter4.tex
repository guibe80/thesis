...
where $f_D$ is the Doppler frequency, $C$ is the speed of the sound assumed to be \SI{1540}{\meter\per\sec}, $f_0$ is the oscillation frequency of the Doppler instrument in this case \SI{8}{\mega\hertz} and $\theta$ the angle between the ultrasound sensor and the target vessel which is \SI{45}{\degree}.
...
First, as explained in equation \ref{eq:dvdr} changes of impedances are equivalent to a fluctuation of volume.  The impedance calculated from equations \label{eq:impedance} and \label{eq:zipg} is the information needed to put into this Nyober's equation.

There are two methods to calculate blood flow. One of them is using venous occlusion plethysmography. Another one is the analysis of the dynamic changes of the impedance waveform. Both methods require using the impedance derivative over time $dR/dt$. However, back projection of the Nyober's equation was used calculating flow in the impedance AC signal. The following section explains in detail the computation of these values.  
...
Making these values more meaningful, they need to be converted in a time interval per minutes. Also, the limb blood flow should be expressed in units of ml per 100 ml of limb section. In this case, the equation can be declared as equation \ref{eq:Qlimb}.

\begin{align}
\label{eq:QLim}
\dot{Q_L} = \bigg( \frac{1}{R_B} \bigg) \bigg( \frac{\Delta R_B}{\Delta t} \bigg) \times 60 min \times 100 ml
\end{align} 

where $\dot{Q_L} $ is the blood flow of the limb expressed as $ml.min^{-1}.100 ml{^-1}$, $R_B$ is the impedance base value, $\Delta R_B$ is the change of impedance within ${\Delta t}$.
