...
As noted before, at the beginning of the study physiological measurements were taken from the participants. Data such age, sex and arm to heart distance were collected initially. The table \ref{tbl:physiological} summarise the information provided by each of the participants. In total, three female and six male participants took part of the study. Their ages ranged between \SIrange{23}{37}{year-old} (mean \num{29.12(494)}). 
...
One of the aims of this experiment is to look into iPG waveform alterations when there is an obstruction in the blood flow in the forearm. It is possible to restrict the blood circulation to this area by applying a mechanical blockage in the upper arm using an standard inflatable blood pressure instrument. The device used necessitates being pumped manually using an inflation bulb. A gauge in the instrument indicating the cuff pressure level. The cuff was secured to the left biceps of the participant. 
...
%I decidede to use the 5.19 because the 60 is a source of error. Whereas HR is more accurate. \mynote{Confirm this equation by referencing it to the paper. Becuase I'm using the 5.14 to calculate the blood flow not the 5.15. In fact, I'm using equation 5.19 to calculate the BF}
...
Following this, one must extrapolate the reference points from the APA pulse given by the Nyboer back-projection \cite{montgomery2011segmental}. This value is equivalent to $dR/dt$ in other blood flow equations but instead two reference points are used. The equation \ref{eq:exht} shows the mathematical operation used to calculate this value.
