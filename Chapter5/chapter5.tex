...
On the other hand, it appears that the median value of participant 6 was heavily affected by a sudden change of the basal impedance signal towards the end of the test in venous occlusion. In fact, As table \ref{tbl:blood_flow:region2} shows there are extremely high values in the blood flow up to (\SI{10}{\ml\per\min 100\ml}, this clearly was caused by movement effect. 
...
In fact, several studies \mynote{Add a reference to a study about AC blood flow estimation} have demonstrated that using is possible to calculate blood flow from the plethysmography waveform. In this case, the change of impedance used to perform this calculation occurs between the foot of the wave and the systolic peak. This $\Delta Z$ is used to calculate blood flow by also applying Nyober's equation \ref{eq:Nyober}. 

%********************************** % Section 5.5.1 ******************************************
\subsection{Blood flow beat by beat during baseline}
\label{section5.5.1}
The plethysmographic baseline signals shown in figure \ref{fig:iPG_venous_baseline} and \ref{fig:iPG_arterial_baseline} were used to calculate the blood flow before occlusive actions. These correspond to regions 1, 3 and 5. The quality of the signal, meaning a good shape of the waveform also depends on added noises to the signal from external sources.  This noise sources may affect the systolic peak which is used to calculate the blood flow. 
