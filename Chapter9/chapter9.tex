...
The LDF device provided raw data in Volts which was converted into BPU units. This conversion was possible by applying equation \ref{eq:BPU} to the data collected.  As explained in section \ref{section:ldf}, the result produced is in arbitrary units which represent blood cell movement under the skin. Therefore, it illustrates the blood flow scale. BPU is the product of the mean number of moving blood cells in the small volume under the probe and the average velocity of moving red blood cells in. Ition of the forearm.  The Lre converted into the device's units (BPU) using the forearmequation specified in \ref{eq:BPU}.                                                                                             e represents the smoothed signal averaged per heartbeat. The shaded area describes the occlusionds of data to the resultant plot. That plot shows that the movement of the cell is affected when an occlusion occurs. It mduring the experimen. The LDF signal can be quite susceptible to noises; this explains the massive peaks seen during some the measurements. Especially participants 1 and 3 depicted a significant number of high peaks. From the qualitative point of view, it is explicit that theavily impacted some parts of the data. Such as in participants 1 and 4. cell movement wasaffected during each occlusion when compared to the baseline data previous to the flow restriction.  
...
Nevertheless, a trend can be noticed in all the participants. During the occlusion in region 2, a slight decline in cell movement can be noticed. It showeems thatn the vnous occlusioprevious baseline data. It seems that the blockage of the venous return slowed down the movement of RBC in the capillaries. Afte, a peak can be seenthe restitution of the bloflow depicted a Gaussian shape peak in most of the participants. This hyperaemic effect shows an acceleration of tockage event.                                              r effecresults can be seenexamined during the pae cells speed reduced compared to the previous baseline (region 3). Then, creating a similar shape as the one seen during venous occlusion. After Cs can be detailed depicting a similar Gaussian shape but with a higher peak. Nevertheless, participant 8 seems to be an exception to the rule. His BPU tends\nknote{not sure if tends is good here} toreadings increased slightly during the transition ofrom region 3 to 4. Moreover, contrary to the other signals in the transition from partial arterial occlusion to baseline in region 5, his BPU decreased. These results obtained from this partaker also could explain some of the adverse readings captured by the iPG signals.

Lastly, during total occlusion, all the mean BPUs of the participants reduced considerably. Then later, when the tourniquet was released, the rush of blood flow can be noticed in the hyperaemic effect registered by the instrument, which was also detectrecorded by others instruments.             e \ref{tbl:LDF flow} reviews the values of the mean BPU data obtained. The results declared there,in the table are in complete agreement with the figure previously analysed.
